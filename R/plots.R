...
# CN plot effort, yield, revenue and profits when fleetDynamics == TRUE, by fleet 
...
plotYield_CN <- function (sim){ 
  ...
  p <- ggplot(y) + geom_line(aes(x = time, y = value, group = gear)) +
    scale_y_continuous(name = "Yield [g]") + 
    scale_x_continuous(name = "Year")+
...
# CN fleetDynamic version. this is different because you changed getFMort
...
plotFMort_CN <- function(sim, species = dimnames(sim@n)$sp,
  ...
  if(length(time_range > 1)){ # if a time range is specified, do the mean 
    f_time <- rowSums(f_time, dims = 3) 
    # fmort should not go above 1 but it could as I am summing moralities from fleets 
    # this is not happening in analysis for JAE paper but could happen if effort steeply increases.  
    # Should change in project? add matrix end of old code
...
plotPPMR<-function(object=object, grid=T, observed=FALSE, prey=dimnames(object@diet_comp)$prey, 
  ...
  plot_dat$obs_PPMR<- object@params@species_params$obs_beta[ma] 
