...
getAvailEnergy <- function(object, n, n_pp, n_bb, n_aa) { 
    ...
    if (length(object@ft_pred_kernel_e) == 1) {
        ...
        # AA adding on temperature
        ## First for the multiple background spectra I need to replace n_pp with all spectra multiplied by their availability 
        #first get the available plankton spectrum food. For this I convert the availability vector into a one column matrix, 
        # so I can use matrix multiplication with n_pp. 
        # This will give a matrix of species x size groups in the full spectrum (species and background)
    ...
    # partial AA adding on benthic spectrum
    ...
    # now we can simply add these matrices because their dimensions should be the same. 
    # This is because n_pp, n_bb and n_aa include the full spectrum, but values are zero above the maximum plankton and benthos and algal size     
    prey_backgr <- pl_food + ben_food + alg_food
...
getFMortGear_CN <- function(object, effort, time_range) {
  ...
  # similar to getFMortGear_CN above but only used in case of fleetDynamics (simplified to exclude all alternatives). 
  ...
  out2[] <- c(object@catchability) * c(object@selectivity) # fishing mortality fleet X species X w - no effort to calculate 'potential' yield for fleetDynamic eqn 
...
getFMort_CN <- function(object, effort, time_range, drop=TRUE){
  ...
  # similar to getFMort above but only used in case of fleetDynamics (simplified to exclude all alternatives).
  ...
  f_mort_gear_FL <- getFMortGear_CN(object, effort)$out2 # fishing mortality fleet X species X w - no effort to calculate 'potential' yield for fleetDynamic eqn  
...
getSMort <- function(object, n, n_pp, n_bb, n_aa, intakeScalar, metScalar,
        ...
        # this happens because mu_S is NA   
        # i.e., when feeding_level or intake_max or metab in getEReproAndGrowth are NA
        
        x <- t(t(mu_S)/(0.1*object@w)) # apply the mortality formula to the whole matrix
        #remember, 0.1 is a parameter here, which is a scaling constant on how negative e translates to starvation mortality. 
        # For a 100g fish with a negative e of -1, it will give starvation value of 0.1. 
        # For a 10 g fish with e of -1, it will give mortality of 1. 
        # This seems reasonable for a start, but a more conmplex relationship could be explored in the future 
...
getEGrowth <- function(object, n, n_pp, n_bb, n_aa, intakeScalar, metScalar,
    ...
    # AA adding growth costs 
    e_growth <- (e - e_repro) * object@species_params$alpha_g # 0.6: species parameter (default 1)
